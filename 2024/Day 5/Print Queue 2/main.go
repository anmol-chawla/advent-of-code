...
func comparator(left, right int, numbers_before_map map[int][]int) {
...
func main() {
	...
	comparator := func(left, right int) int {

		allowed_before_list := numbers_before_map[left]
		allowed_after_list := numbers_after_map[left]

		if slices.Contains(allowed_before_list, right) {
			return -1
		}

		if slices.Contains(allowed_after_list, right) {
			return 1
		}
	...
	for i := range number_lines {
		...
		if incorrect {
			slices.SortFunc(number_lines[i], comparator)
